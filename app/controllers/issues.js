...
} = require("../data/wcag_criteria");
const { db } = require("../db");
const {
    getIssueResources,
    createResource,
    deleteResource,
} = require("../data/issue_resources");
...
const index = async(req, res) => {
    try {
        ...
        if (user.role === "super_admin") {
            ...
            const result = await issuesData.getDepartmentOpenIssues(
                departmentId,
                filters.page,
                filters.limit
            );
        ...
        if (filters.wcag_level) {
            ...
            const selectedLevels = Array.isArray(filters.wcag_level) ?
                filters.wcag_level : [filters.wcag_level];
            issues = issues.filter((issue) =>
                selectedLevels.includes(issue.wcag_level)
            );
        }

        if (filters.service_id) {
            issues = issues.filter(
                (issue) => issue.service_id === filters.service_id
            );
        }
...
const issuesByCriterion = async(req, res) => {
    ...
    res.render("department_admin/issues/criteria", {
        issues,
        wcagCriteria,
        user: req.session.user,
    });
...
async function handleUpdateIssue(req, res) {
    try {
        const { serviceId, id } = req.params;
        const issueTypes = req.body.issue_types ?
            Array.isArray(req.body.issue_types) ?
            req.body.issue_types : [req.body.issue_types] : [];
        const selectedWcagCriteria = req.body.wcag_criteria ?
            Array.isArray(req.body.wcag_criteria) ?
            req.body.wcag_criteria : [req.body.wcag_criteria] : [];
        ...
        await issuesData.updateIssue(
            id,
            issueData,
            selectedWcagCriteria,
            issueTypes
        );
...
async function addComment(req, res) {
    try {
        ...
        if (!comment || comment.trim() === "") {
            return res.redirect(
                `/services/${serviceId}/issues/${issueId}?error=comment_empty`
            );
        }
...
async function handleDeleteComment(req, res) {
    try {
        ...
        if (
            ...
            return res.redirect(
                `/services/${serviceId}/issues/${issueId}?error=permission`
            );
        }

        await deleteComment(commentId);
        res.redirect(
            `/services/${serviceId}/issues/${issueId}?success=comment_deleted`
        );
    } catch (error) {
...
const requestRetest = async(req, res) => {
    try {
        ...
        if (!process.env.GOVUK_NOTIFY_API_KEY ||
            ...
            return res.redirect(
                `/services/${serviceId}/issues/${id}?error=notify_config`
            );
        }
        ...
        res.redirect(
            `/services/${serviceId}/issues/${id}?success=retest_requested`
        );
    } catch (error) {
        console.error("Error requesting retest:", error);
        res.redirect(
            `/services/${req.params.serviceId}/issues/${req.params.id}?error=retest_failed`
        );
    }
};
...
const completeRetest = async(req, res) => {
    try {
        ...
        res.redirect(
            `/services/${serviceId}/issues/${id}?success=retest_completed`
        );
    } catch (error) {
        console.error("Error completing retest:", error);
        if (error.response && error.response.data) {
            console.error("Notify API error details:", error.response.data);
        }
        res.redirect(
            `/services/${req.params.serviceId}/issues/${req.params.id}?error=retest_complete_failed`
        );
    }
};
...
async function deleteIssueResource(req, res) {
    try {
        ...
        res
            .status(500)
            .render("error", { error: "Failed to delete issue resource" });
    }
}
