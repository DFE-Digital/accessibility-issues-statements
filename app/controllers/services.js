    ...
    try {
        ...
        if (filters.search) {
            ...
            openIssues = openIssues.filter(issue =>
                ...
                (issue.wcag_level && issue.wcag_level.toLowerCase().includes(searchTerm))
            );
        }
...
const assignIssue = async(req, res) => {
    try {
        ...
        if (!assign_to && !new_user_email) {
            return res.redirect(`/services/${serviceId}/issues/${issueId}/assign?error=no_user_selected`);
        }
        ...
        res.redirect(`/services/${req.params.serviceId}/issues/${req.params.issueId}?error=assign_failed`);
    }
};
...
async function assignToTeam(req, res) {
    try {
        ...
        if (req.session.user.role !== 'super_admin' &&
            (!req.session.user.department || req.session.user.department.id !== issue.department_id)) {
