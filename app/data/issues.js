...
async function getIssue(issueId) {
    ...
    const createdBy = await db('users')
        ...
        .where('users.id', issue.created_by)
        .first();
    createdBy.name = `${createdBy.first_name} ${createdBy.last_name}`;
    ...
    if (issue.assigned_to) {
        const assignedUser = await db('users')
            .select('users.first_name', 'users.last_name', 'users.email')
            .where('users.id', issue.assigned_to)
            .first();
        if (assignedUser) {
    ...
    return {
        ...
        wcag_level: getWcagLevel(issue.wcag_tags)
    };
}
...
async function createIssue(issueData, wcagCriteria = [], issueTypes = []) {
    ...
    try {
        ...
        const [issue] = await trx('issues').insert(issueData).returning('*');

        if (wcagCriteria.length > 0) {
            const criteriaToInsert = wcagCriteria.map(criterion => ({
                issue_id: issue.id,
                wcag_criterion: criterion,
                department_id: service.department_id
            }));
            await trx('issue_wcag_criteria').insert(criteriaToInsert);
        }

        if (issueTypes.length > 0) {
            const typesToInsert = issueTypes.map(type => ({
                issue_id: issue.id,
                type: type,
                department_id: service.department_id
            }));
            await trx('issue_types').insert(typesToInsert);
        }
...
function getWcagLevel(tags) {
    ...
    const levels = tags.map(tag => tag.level);
...
/**
 * Request a retest for an issue
 * @param {string} issueId - Issue ID (UUID)
 * @returns {Promise<Object>} Updated issue
 */
async function requestRetest(issueId) {
    return db('issues')
        .where({ id: issueId })
        .update({ retest_request: true, updated_at: new Date() });
}

/**
 * Mark a retest request as complete
 * @param {string} issueId - Issue ID (UUID)
 * @returns {Promise<Object>}
 */
async function completeRetest(issueId) {
    return db('issues')
        .where({ id: issueId })
        .update({ retest_request: false, updated_at: new Date() });
}
